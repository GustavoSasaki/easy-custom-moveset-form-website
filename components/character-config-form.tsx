...
import { Download, Copy, Check, Sparkles, ChevronDown, HelpCircle } from "lucide-react"
...
const presets: { name: string; config: Partial<CharacterConfig> }[] = [
  ...
  },
  {
    ...
    config: {
      ...
      ground_pound_jump_on: true,
      moveset_description: "yoshi flutter, triple jump twirling, ground pound jump",
    },
  },
]
...
export function CharacterConfigForm({ translations: t }: { translations: Translations }) {
  ...
  const generateLuaCode = useCallback(() => {
    ...
    const lines: string[] = ["{", `    -- ${config.name} custom character config`, `    name = '${config.name}',`]
    ...
    // Twirling
    if (config.back_flip_twirling_on || config.side_flip_twirling_on || config.triple_jump_twirling_on) {
          ...
          <Card className="border-2 border-border">
            ...
            <CardContent className="space-y-2">
              <Input
                ...
                className="border-2 font-medium"
              />
              <p className="text-xs text-muted-foreground">({t.optional}) - {t.autoGenerated}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="sticky bottom-0 z-10 -mx-4 bg-background/95 backdrop-blur-sm p-4 border-t-2 border-border lg:relative lg:mx-0 lg:bg-transparent lg:backdrop-blur-none lg:p-0 lg:border-t-0 lg:w-96 lg:flex-shrink-0">
          <div className="lg:sticky lg:top-4 space-y-3">
            ...
            <div className="flex flex-col gap-2 sm:flex-row lg:flex-col">
              ...
              <Button variant="outline" onClick={handleCopy} className="flex-1 border-2 bg-transparent font-bold uppercase tracking-wide">
